...
const pluralSuppliers = (n) => n === 1 ? '1 поставщик' : n >= 2 && n <= 4 ? `${n} поставщика` : `${n} поставщиков`;

/* Раскрытая строка с поставщиками */
const SupplierInfo = ({ supplier, uom }) => {
    const [open, setOpen] = useState(false);
    const priceRange = supplier.price_range?.some(p => p !== 0) ? supplier.price_range
        ...
        <div style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid var(--hairline)', background: 'var(--canvas)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span style={{ fontFamily: 'var(--sans)', fontSize: 11, background: 'var(--surface-card)', color: 'var(--muted)', borderRadius: 9999, padding: '2px 8px' }}>
                    {supplier.total_supplies} {supplier.total_supplies === 1 ? 'поставка' : supplier.total_supplies < 5 ? 'поставки' : 'поставок'}
                </span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 6 }}>
                <div style={{ background: 'var(--surface-soft)', borderRadius: 6, padding: '6px 8px' }}>
                    ...
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500, color: 'var(--ink)' }}>{(supplier.avg_price || 0).toFixed(2)} ₽/{uom}</div>
                </div>
                <div style={{ background: 'var(--surface-soft)', borderRadius: 6, padding: '6px 8px' }}>
                    ...
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500, color: 'var(--ink)' }}>
                        {hasRange ? `${priceRange[0].toFixed(2)}–${priceRange[1].toFixed(2)} ₽` : `${(supplier.avg_price || 0).toFixed(2)} ₽`}
                    </div>
                </div>
            </div>
            <div style={{ background: 'var(--surface-soft)', borderRadius: 6, padding: '6px 8px', marginBottom: supplier.last_supplies?.length ? 6 : 0 }}>
                <div style={{ fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--muted)', marginBottom: 2 }}>Всего</div>
                <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500, color: 'var(--ink)' }}>{supplier.total_quantity?.toLocaleString('ru-RU')} {uom}</div>
            </div>
            {supplier.last_supplies?.length > 0 && (
                <>
                    <button onClick={() => setOpen(o => !o)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--muted)', padding: 0, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <ChevronRight size={11} style={{ transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 150ms' }} />
                        Последние поставки
                    </button>
                    {open && (
                        <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {supplier.last_supplies.map((s, i) => (
                                <div key={i} style={{ background: 'var(--surface-soft)', borderRadius: 6, padding: '5px 8px', display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--muted)' }}>{s.date}</span>
                                    <span style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--ink)' }}>{s.price.toFixed(2)} ₽ · {s.quantity.toLocaleString('ru-RU')} {uom}</span>
                                </div>
                            ))}
                        </div>
                    )}
...
const COLUMNS = [
    ...
    { key: 'total_usage',    label: 'Количество' },
    { key: 'shipments_count', label: 'Отгрузки' },
    { key: 'suppliers_count', label: 'Поставщики' },
];
...
const MaterialTable = ({ materials, loading, pagination, sortField, sortOrder, onSort, onPageChange, onMaterialClick, filters }) => {
    ...
    const fetchDetails = async (id) => {
        ...
        try {
            ...
            const data = await materialsApi.getDetails(id, qs);
                        ...
                        {loading && materials.length === 0 ? (
                            ...
                            return [
                                ...
                                >
                                    ...
                                    <td style={tdStyle}>{row.total_usage?.toLocaleString('ru-RU')} {row.uom}</td>
                                    <td style={tdStyle}>{row.shipments_count?.toLocaleString('ru-RU')}</td>
                                    <td style={tdStyle}>{row.suppliers_count ? pluralSuppliers(row.suppliers_count) : '—'}</td>
