...
RowActions.propTypes = { row: PropTypes.object.isRequired };
...
function OrderCard({ row }) {
    return (
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--hairline)' }}>
            ...
            <div style={{ marginTop: 8 }}><RowActions row={row} /></div>
        </div>
    );
}
OrderCard.propTypes = { row: PropTypes.object.isRequired };

export default function SiteOrdersTable({ rows, loading, sort, onSortChange, isMobile }) {
    if (isMobile) {
        ...
        return (
            <div style={{ border: '1px solid var(--hairline)', borderRadius: 10, overflow: 'hidden' }}>
                {rows.map(row => <OrderCard key={row.order_id} row={row} />)}
            </div>
        );
    }
