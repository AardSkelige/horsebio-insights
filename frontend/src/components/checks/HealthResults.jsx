...
const STAT_HINTS = {
    ...
    // Ключ — имя показателя из вывода скрипта, то есть данные: переименовать его нельзя,
    // иначе подсказка перестанет находиться. К норме приводим только текст.
    'Не ВБ/Озон': 'Заказы не от Ozon и Wildberries — этим монитором не обрабатываются.',
...
export default function HealthResults({ scriptId, runId, running, onExceptionChange }) {
    ...
    // Возвраты Ozon/Wildberries: стат-карточки дублируют список ниже — прячем их (робот-режим
