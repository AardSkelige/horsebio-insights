...
export const SCRIPT_META = {
    ...
    horsebio_returns: {
        ...
        how: 'Робот спрашивает у маркетплейсов список возвратов и сам создаёт черновики',
        hint: 'Раньше робот смотрел только на статус заказа — и видел половину: если покупатель '
            + 'выкупил товар и вернул позже, заказ так и оставался «Доставлен», а возврат мы не замечали. '
            + 'Теперь источник — сами маркетплейсы: робот берёт у них список возвратов, находит отгрузку '
            + 'и заводит черновик. Так видно, какой товар должен вернуться и сколько денег в нём. '
            + 'Черновик проводят, когда товар физически приходит на склад.',
    },
    horsebio_returns_ozon_enrich: {
        Icon: MapPin,
        what: 'У каждого черновика возврата Озон видно, где товар и что с ним делать',
        how: 'Робот берёт у Ozon статус возврата и ставит его на документ',
        hint: 'Черновик возврата сам по себе не говорит, где коробка. Робот спрашивает Ozon и ставит '
            + 'статус: «Едет к нам» — ждём; «Забрать в ПВЗ» — лежит в пункте выдачи, надо съездить; '
            + '«У нас — разобрать» — товар на складе, Лере проверить и провести; «Завис в пути» — больше '
            + 'месяца в дороге, писать в поддержку; «Ушёл на склад МП» — к нам не приедет, документ '
            + 'закроется сам через 30 дней. Черновики без номера отправления (ФБО, заведённые вручную) '
            + 'робот не трогает.',
    },
    horsebio_returns_wb_enrich: {
        Icon: Truck,
        what: 'У каждого черновика возврата ВБ видно, где товар и что с ним делать',
        how: 'Робот берёт статус из отчёта Wildberries и ставит его на документ',
        hint: 'Считалось, что из API Вайлдберриз маршрут возврата не достать — оказалось, достать можно: '
            + 'отчёт «Возвраты товаров» отдаёт статус и пункт выдачи. Статусы те же, что у Озона: едет к нам, '
            + 'забрать в ПВЗ, у нас — разобрать, завис в пути. Сопоставление идёт по номеру задания ВБ из '
            + 'описания заказа; возвраты ФБО номера не имеют, их робот пропускает — их оформляют вручную.',
    },
    horsebio_pending_returns: {
        Icon: PackageOpen,
        what: 'Черновики возвратов не висят без товара дольше 30 дней',
        how: 'Считаем возраст и сумму каждого непроведённого возврата, группируем по статусу',
        hint: 'Черновик возврата — штатное состояние: документ проводят, когда товар физически '
            + 'дошёл до склада. Проверка отвечает, сколько денег в этом висит и что требует действия: '
            + 'лежит в пункте выдачи (съездить забрать), уже у нас (разобрать и провести), завис в пути '
            + '(писать в поддержку маркетплейса). Возраст считаем от начала возврата у маркетплейса, '
            + 'а не от даты документа — черновик мог быть заведён задним числом. Возвраты, осевшие на '
            + 'складе маркетплейса, в подсчёт не идут: этот товар к нам не приедет.',
    },
    horsebio_deadlines: {
        Icon: CalendarClock,
