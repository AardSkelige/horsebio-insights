...
4. **Не выложено на сайт** — товар на складе лежит, а карточки на витрине нет.
   Срок при этом идёт, то есть каждый день ожидания — минус день продаж.
5. **Не проставлен «Годен до»** — товар на складе есть, а даты в карточке нет.
...
Первые четыре правила намеренно не пересекаются: действие у них одно — тот же
файл, — и два уведомления об одной банке читались бы как шум. Пятое стоит
отдельно и может прийти вместе с ними: его действие адресовано другому человеку
...
def _not_published(position):
    """Товар на складе, а карточки на витрине нет.

    Пока карточки нет, партия не продаётся, но срок идёт — то есть ожидание
    прямо съедает окно продаж. У масла оно и так вышло коротким: месяц вместо
    двух, потому что до конца срока оставалось меньше обычного.
    """
    quantity = int(position["quantity"])
    days_left = position["days_left"]
    return Notification(
        key=f"discounted:not-published:{position['id']}",
        level=WARNING,
        title=f"{_short(position['name'])} — не выложено на сайт",
        body=f"На складе {quantity} шт, до конца срока {days_left} дн — товар не продаётся",
        action=f"{FILE_ACTION} — карточка заведётся скрытой, останется её открыть",
        # Ни количество, ни дни в отпечаток не идут: они меняются сами,
        # а дело всё то же — карточки на витрине нет
...
def discounted_notifications():
    for position in positions_snapshot():
        ...
        # published is False, а не «не published»: None означает, что фид сайта
        # не ответил, и звать выкладывать то, что уже выложено, — плохая идея
        elif quantity > 0 and position.get("published") is False and position["state"] == STATE_OK:
            yield _not_published(position)
