...
SCRIPTS_CONFIG = [
    ...
    {
        'id': 'horsebio_discounted_snapshot',
        'topic': 'Обновление данных',
        'name': 'Снимок уценки',
        'account': 'HorseBio',
        'schedule': 'Каждые 30 мин',
        'description': 'Пересобирает раздел «Уценка» из МойСклад в базу: страница и уведомления читают снимок, а не ходят в МойСклад при каждом открытии (Django-команда build_discounted_snapshot)',
        'script': '/app/manage.py',
        'args': ['build_discounted_snapshot'],
    },
    {
        'id': 'horsebio_fbo_stock_snapshot',
        'topic': 'Обновление данных',
        'name': 'Снимок остатков для FBO',
        'account': 'HorseBio',
        'schedule': 'Каждые 30 мин',
        'description': 'Пересобирает раздел «Остатки для FBO» из МойСклад в базу: страница читает снимок, а не ходит в МойСклад при каждом открытии (Django-команда build_fbo_stock_snapshot)',
        'script': '/app/manage.py',
        'args': ['build_fbo_stock_snapshot'],
    },
    {
        'id': 'horsebio_fbo_snapshot',
        'topic': 'Обновление данных',
        'name': 'Снимок FBO-заказов',
        'account': 'HorseBio',
        'schedule': 'Каждые 30 мин',
        'description': 'Пересобирает раздел «FBO Заказы» из МойСклад в базу: страница читает снимок, а не ходит в МойСклад при каждом открытии (Django-команда build_fbo_snapshot)',
        'script': '/app/manage.py',
        'args': ['build_fbo_snapshot'],
    },
    {
