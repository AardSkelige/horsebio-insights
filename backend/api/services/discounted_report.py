# api/services/discounted_report.py
"""
Расчёт раздела «Уценка» — всё, что ходит в МойСклад и считает.
...
Расчёт жил во вьюхе (api/views/discounted.py) до 11.09.2026 — вместе с ручками
раздела. Разъехались, когда снимок стал собирать команда по расписанию: команде
нечего делать во вьюхе, а вьюхе незачем знать, как ходить в МойСклад.
"""
...
from api.services import ozon_stock, section_snapshots, site_feed
...
def ms_get_all_pages(path, params=None):
...
def _resolve_refs():
    ...
    folders = ms_get("/entity/productfolder", {"filter": f"name={FOLDER_NAME}", "limit": 10}).get("rows", [])
    ...
    attributes = ms_get("/entity/product/metadata/attributes").get("rows", [])
...
def _build_positions(refresh=False, with_days_on_stock=True, with_ozon=True):
    ...
    for product_id, product in by_id.items():
        ...
        retail = price_of(product, RETAIL_PRICE_NAME)
