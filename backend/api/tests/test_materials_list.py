...
Показатели считаются подзапросами, и оба свойства этого решения нужно
удерживать проверками. Числа — потому что агрегаты по двум разным связям
легко размножить join'ом: материал с двумя поставками и тремя расходами
покажет утроенную поставку, и заметить это на глаз в таблице нельзя.
Цена — потому что прежняя реализация обходила материалы циклом, и запросов
получалось втрое больше, чем строк в справочнике.
"""
