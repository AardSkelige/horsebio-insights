...
STALE_HOURS = 36
# Норма полного архива — около 10 МБ (почти весь объём — дамп базы), и она
# медленно растёт вместе с зеркалом МойСклада. Порог ловит архив, похудевший
# вдвое: формально успех, фактически дамп снялся не целиком.
#
# Стоял 10 при норме 10,3 — запас 3%, и 09.09.2026 проверка сработала
# на обычном колебании сжатия (9,9 МБ при тех же 56 таблицах с данными).
# Порог, который срабатывает от шума, перестают читать.
MIN_FULL_MB = 6
...
def build_payload(state, unavailable):
    ...
    if state.get("ok") and 0 < full_mb < MIN_FULL_MB:
        categories.append(category(
            ...
            f"Норма — около 10 МБ, сейчас {full_mb} МБ. Прогон завершился успешно, "
            ...
            f"{full_mb} МБ при ожидаемых ~10 МБ",
        ))
