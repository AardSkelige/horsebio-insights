...
* Вместо пустого XML сайт отвечает JSON-ошибкой
  `{"error":{"message":"Bad Request","code":400}}` с HTTP 200. Отличить по ней
  «очередь пуста» от «сейчас не отдам» нельзя: 25.08.2026 сразу после
  mode=success запросы стали получать этот ответ, хотя в админке заказы
  числились «Не выгружен» — похоже на ограничение частоты обмена. Поэтому такой
  ответ считаем не пустой очередью, а отказом (см. `refused`): если он
  затягивается, сверка обязана об этом сказать, а не показывать зелёное.
...
class SiteOrdersExport:
    ...
    def __init__(self, url: str = None, login: str = None, password: str = None):
        ...
        self.refused = False      # последний fetch() получил отказ вместо XML
    ...
    def fetch(self) -> list:
        ...
        if not body.startswith("<"):
            # Не XML — сайт отдавать отказался. Падать не надо (сверимся по
            # сохранённой копии), но и делать вид, что заказов нет, нельзя.
            self.refused = True
            return []
        self.refused = False
