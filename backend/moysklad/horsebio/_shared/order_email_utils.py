...
# Замок и чтение-запись журнала переехали в базу (order_email_store): файла
# больше нет, а процессов по-прежнему трое. Здесь оставлены имена, которыми
# их зовут роботы и страница «Заказы сайта».
from order_email_store import (  # noqa: E402
    forget_order, last_checked_at, load_state, save_state, state_lock,
)
...
def delivery_address(latest: dict) -> str:
    """Адрес доставки из письма — под любым именем поля.

    Имя поля задаёт CMS сайта, и оно не одно: до 08.09.2026 все заказы приходили
    с «Полный адрес доставки», а заказ 22298118 (СДЭК «Посылка до ПВЗ») —
    с «Адрес доставки». Пустой адрес стоит дорого: заказ уходит в МойСклад без
    адреса, робот накладных СДЭК его не оформляет, и оплаченный заказ висит.
    Поэтому берём первое непустое поле, в названии которого есть «адрес».
    """
    for name, value in (latest.get("delivery_field") or {}).items():
        if "адрес" in name.lower() and (value or "").strip():
            return value.strip()
    return ""
