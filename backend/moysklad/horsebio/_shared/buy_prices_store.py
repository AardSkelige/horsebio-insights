...
class DbStore:
    ...
    def save(self, state: dict) -> None:
        ...
        with transaction.atomic():
            ...
            #
            # Но удаляем только то, что старше самого раннего прогона в списке.
            # Прогон новее, которого в нашей истории нет, — это чужая запись:
            # ручной запуск скрипта пересекается с плановым, оба прочитали
            # историю до, и сохраняющий вторым иначе снёс бы прогон первого.
            # Историю изменений закупочных цен восстановить неоткуда.
            oldest = min(row.date for row in rows)
            BuyPriceSyncRun.objects.filter(date__lt=oldest).delete()
