#!/usr/bin/env python3
"""
Мониторинг сроков оплаты по отгрузкам с отсрочкой платежа — HorseBio.
...
def get_counterparty_map() -> dict:
    """Возвращает {agent_id: {"days": int, "name": str}} только для контрагентов с отсрочкой."""
...
def get_demands_for_counterparties(agent_ids: list) -> list:
    demands = []
    for agent_id in agent_ids:
        agent_href = f"https://api.moysklad.ru/api/remap/1.2/entity/counterparty/{agent_id}"
        batch = get_all_pages("/entity/demand", {
            "filter": f"applicable=true;agent={agent_href}",
            "expand": "attributes",
            "order": "moment,asc",
        })
...
def get_reports_for_counterparties(agent_ids: list) -> list:
    """Полученные отчёты комиссионера — именно по ним возникает долг комиссионера."""
    reports = []
    for agent_id in agent_ids:
        agent_href = f"https://api.moysklad.ru/api/remap/1.2/entity/counterparty/{agent_id}"
        batch = get_all_pages("/entity/commissionreportin", {
            "filter": f"applicable=true;agent={agent_href}",
            "order": "moment,asc",
        })
...
def analyze_documents(docs: list, counterparty_map: dict, show_all: bool, doc_kind: str = "demand",
    ...
    for d in docs:
        ...
        if individual_days:
            ...
            agent_name = counterparty_map.get(agent_id, {}).get("name", "—") if agent_id else "—"
        else:
            cp = counterparty_map.get(agent_id)
            if not cp:
                continue
            days = cp["days"]
            agent_name = cp["name"]
...
def main():
    parser = argparse.ArgumentParser(description="Мониторинг сроков оплаты — HorseBio")
    ...
    header = f"\n{'=' * 70}\nМОНИТОРИНГ ОТСРОЧЕК ПЛАТЕЖА (HorseBio) — {now_str}\n{'=' * 70}"
    print(header)

    print("Загружаем контрагентов...")
    counterparty_map = get_counterparty_map()
    ...
    demands = get_demands_for_counterparties(list(counterparty_map.keys()))
    print(f"  Проведённых отгрузок: {len(demands)}")

    print("Загружаем отчёты комиссионера...")
    reports = get_reports_for_counterparties(list(counterparty_map.keys()))
